...
namespace DoSomethingClient
{
    public class DomainAssemblyLoader : MarshalByRefObject
    {
        ...
        public Result Load(string assemblyString, Input data)
        {
            ...
            {
                bool implementInterface = item.GetInterfaces().Contains(typeof(IDoSomething));
                bool hasAttribute = Attribute.IsDefined(item, typeof(DoSomethingAttribute));

                if (implementInterface && hasAttribute)
                {
                    doSomethingService = (IDoSomething)Activator.CreateInstance(item);
                    break;
                }
            }
        ...
        public Result LoadFile(string path, Input data)
        {
            ...
            foreach (var item in types)
            {
                ...
                if (hasAttribute)
                {
                    ...
                    result = (Result)mi.Invoke(Activator.CreateInstance(item), data.Users);
                }
            }            
